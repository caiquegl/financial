      ...
      <Svg width={width} height={width}>
        <G transform={`translate(${halfWidth}, ${halfWidth})`}>
          {data.map((item, index) => {
            ...
            return (
              <>
                <Path key={index} d={path} fill={colors[index % colors.length]} strokeWidth={strokeWidth} />
                <SvgText x={halfWidth} y={halfWidth - (index * 35)} textAnchor="middle" fontSize={16}>{item.title}</SvgText>
              </>
            );
          })}
        </G>
