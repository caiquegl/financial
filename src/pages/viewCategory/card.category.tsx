        ...
        <View style={styles.container}>
            <TouchableOpacity onPress={toggleCard} activeOpacity={0.8} style={styles.header}>
                <View style={[styles.header, {flex: 1}]}>
                    ...
                    <TouchableOpacity onPress={() => deleteCategory(data.id, data.name)} style={{marginLeft: 10}}>
                ...
                {hasSubcategory && <Ionicons name={expanded ? 'chevron-up-outline' : 'chevron-down-outline'} size={24} color="#666" />}
            </TouchableOpacity>
            ...
            <Drawer isOpen={drawerOpen} onClose={() => setDrawerOpen(!drawerOpen)} right={true}>
                <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center' }}>
                    <UpdateCategory id={data.id} is_primary={data.is_primary} name={data.name} sub_category={data.sub_category} reload={reload} />
                </View>
            </Drawer>
        </View>
    );
